    ...
    if k==1
        ...
        switch method   
           case 'STTCGPM'
               ...
               p_k = Fv0 ;
               dk=-theta_k.*Fv0 + beta_k .*d_k_prev + 0.1*((Fv0'*d_k_prev)/(norm(d_k_prev)*norm(p_k)))*p_k;
           case 'FITTCGPM-PRP'
                 w0 = Fv0-Fy0_old;
          ...
          case 'FITTCGPM-NEW'
                w0 = Fv0-Fy0_old;
                U_k = mu *(norm(d_k_prev)^2 + NormFv0^2+ abs(d_k_prev'*w0));
                betak = NormFv0^2/U_k - ((NormFv0^2 * (Fv0'*d_k_prev))/ U_k^2);
                 nuk=norm(Fv0)/max(norm(Fy0_old),(abs(betak))*norm(dk));
                thetak=-0.055*Fv0'*Fy0_old/norm(Fy0_old)^2;  
                dk=-Fv0+0.001*nuk*betak*dk+thetak*Fy0_old;
           case 'IMSMNE'
                ...
                k_a = max(tauk*Normdk*norm(h_k), dk'*h_k);
                betak= (Fv0'*h_k)/(k_a) - ((norm(h_k)^2 * Fv0'*dk)/(k_a^2))  ;
                theta = ((sk'*h_k)/(sk'*sk));
