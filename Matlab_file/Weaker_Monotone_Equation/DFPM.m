...
for k=1:k_max
    ...
    if k==1
        ...
        switch method   
           ...
           case'FITTCGPM-DY'
                 ...
                 dk=-Fv0+0.001*nuk*betak*dk+thetak*Fy0_old;   
          case 'GITDFPA'
               mu =2.5;w=0.2;eta0 =0.1;
               s_k =v0-v0_old ;
               w0 = Fv0-Fy0_old;
               NormFy0 = norm(Fy0_old)^2;
               p_k = dk ;
               Fv0tdk = Fv0'*dk;
               Normpk = norm(p_k);
               U_k = mu *(Normdk2 + NormFv0^2)+ max([Normdk2, NormFy0 , dk'*w0]) ;
               W_k = max(w*Normdk*Normpk,  NormFy0 );
               eta_k = min(eta0 , max(0, (p_k'*(w0 -s_k))/(Normpk^2)));
               beta_k = NormFv0^2/U_k - ((NormFv0^2 * (Fv0tdk))/ U_k^2);
               dk=-Fv0 + beta_k .*dk +eta_k* ((Fv0tdk)/(W_k))*p_k;
            otherwise
    ...
    elseif model==2
        ...
        Fz0_newtdk = -Fz0_new'*dk;
        % check the Armijo-type line search condition
        while Fz0_newtdk < sigma*t*min(phi_1,phi_2*NormFz0_new)*Normdk2 %&& t>10^-6  
            % the Armijo-type line search condition violated
            t = t*tau;
            z0_new = v0+t*dk;
            Fz0_new = feval(nprob,z0_new,1);
